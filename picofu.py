...
class FWUpdate(object):
  ...
  def power_detect(self):
    ...
    if not self.power and not self.power_warned:
      ...
      PIco= self.serial.Serial(port=self.port,baudrate=38400,timeout=0,rtscts=0,xonxoff=0)
  ...
  def serial_check(self):
    ...
    try:
      ...
      PIco = self.serial.Serial(port=self.port,baudrate=38400,timeout=0,rtscts=0,xonxoff=True)
      # empty the input buffer
      for line in PIco:
        pass
      # send dummy command
